...
func _ready() -> void:
	...
	_sounds["slide"]         = _synth_noise(0.18, 0.18)
	_sounds["dive"]          = _synth_tone(880.0, 220.0, 0.18, 0.24)
	_sounds["wall_jump"]     = _synth_tone(440.0, 660.0, 0.12, 0.24)
	...
	_sounds["teleport"]        = _synth_tone(220.0, 1320.0, 0.22, 0.20)
	_sounds["mine_detonate"]   = _synth_noise(0.20, 0.30)
	_sounds["fissure_explode"] = _synth_noise(0.16, 0.28)
	_sounds["subweapon_fire"]  = _synth_tone(560.0, 280.0, 0.10, 0.20)
	_sounds["npc_blip"]        = _synth_tone(660.0, 660.0, 0.04, 0.14)
	_sounds["achievement"]     = _synth_tone(880.0, 1760.0, 0.32, 0.22)
	_sounds["boss_stinger"]    = _synth_tone(110.0, 55.0, 0.36, 0.32)
	_sounds["lock"]            = _synth_tone(220.0, 110.0, 0.18, 0.22)
